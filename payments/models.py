...
class Payment(models.Model):
    ...
    def update_status_based_on_order(self, force_save=False):
        ...
        # Liste des statuts où le paiement doit rester en attente
        PENDING_STATUSES = [
            'UNCONFIRMED', 'CONFIRMED', 'STOCK_VERIFICATION', 
            'PREPARATION', 'PACKAGING', 'WAITING_FOR_PAYMENT', 'SHIPPED'
        ]
        ...
        elif self.order.status in PENDING_STATUSES:
            self.status = 'PENDING'
        elif self.order.status == 'CANCELLED':
            self.status = 'PENDING'
            # On pourrait aussi vider les dates si nécessaire
